...
class InfuraCrawler:
    ...
    def start(self):
        for pairInfo in self.pairInfos:
            ...
            if self.exchange == 'sushiswap' and pairAddress == '0x397FF1542f962076d0BFE58eA045FfA2d347ACa0':
                (token0Reserves, token1Reserves, timestamp) = (1000*10**pairInfo['token0Decimals'], 1100*10**pairInfo['token1Decimals'], 9999999)
                print("{}///{}".format(token0Reserves, token1Reserves))
            else:
                (token0Reserves, token1Reserves, timestamp) = (1000*10**pairInfo['token0Decimals'], 1000*10**pairInfo['token1Decimals'], 9999999)
